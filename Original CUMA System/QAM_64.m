...
numBits = 6 * numSymbols; % 1 symbol 6 bits
...
for experiment = 1:numExperiments
    ...
    for n = 1:Nt
        ...
        %fprintf('input signal for user %d under 64-QAM modulation', n);
        %disp(s_u{n});
    end
    ...
    for u = 1:U
        ...
        for m = 1:numSymbols
            ...
            demodulated_bits{u}(6*m-5:6*m) = [real_bits, imag_bits];
        end
