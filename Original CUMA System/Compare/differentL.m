...
BER_results = zeros(length(modulation_schemes), length(L_values)); % Store BER results
    ...
    for l = 1:length(L_values)
        ...
        for experiment = 1:numExperiments
            ...
            for u = 1:U
                ...
                % Note: N1 and N2 are hardcoded here as example values.
        ...
        % Calculate average BER for this L value and modulation scheme
        average_BER_users = mean(BER_users, 2);
        average_BER = mean(average_BER_users);
        BER_results(m, l) = average_BER;
    end
end
...
markers = {'-o', '-s', '-d', '-^', '-v'};
for m = 1:length(modulation_schemes)
    plot(L_values, BER_results(m, :), markers{m}, 'LineWidth', 2, 'DisplayName', modulation_schemes{m});
end
set(gca, 'YScale', 'log');
