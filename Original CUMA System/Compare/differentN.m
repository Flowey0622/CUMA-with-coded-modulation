...
BER_results = zeros(length(modulation_schemes), length(port_counts)); % Store BER results
...
for m = 1:length(modulation_schemes)
    ...
    for p = 1:length(port_counts)
        ...
        % Calculate average BER for this port count and modulation scheme
        average_BER_users = mean(BER_users, 2);
        average_BER = mean(average_BER_users);
        BER_results(m, p) = average_BER;
    end
end
...
markers = {'-o', '-s', '-d', '-^', '-v'};
for m = 1:length(modulation_schemes)
    plot(port_counts.^2, BER_results(m, :), markers{m}, 'LineWidth', 2, 'DisplayName', modulation_schemes{m});
end
set(gca, 'YScale', 'log');
