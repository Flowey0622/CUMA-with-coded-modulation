...
modulation_schemes = {'BPSK', 'QPSK', '16-QAM', '64-QAM', '256-QAM'}; % different modulations
BER_results = zeros(length(modulation_schemes), length(rho_values)); 
...
for m = 1:length(modulation_schemes)
    ...
    for r = 1:length(rho_values)
        rho = rho_values(r); %current rho

        N1 = 20; 
        N2 = 20; 
        N = N1 * N2; 

        n1 = transpose((0:N1-1)./(N1-1).*10*lamda); 
        n2 = transpose((0:N2-1)./(N2-1).*10*lamda); 
        ...
        for experiment = 1:numExperiments
            ...
            for n = 1:Nt
                ...
                elseif strcmp(modulation_scheme, 'QPSK')
                    numBits = 200; % QPSK 2 bits/symbol
                ...
                elseif strcmp(modulation_scheme, '16-QAM')
                    numBits = 400; % 16-QAM 4 bits/symbol
                ...
                elseif strcmp(modulation_scheme, '64-QAM')
                    numBits = 600; % 64-QAM 6 bits/symbol
                ...
                elseif strcmp(modulation_scheme, '256-QAM')
                    numBits = 800; % 256-QAM 8 bits/symbol
            ...
            for u = 1:U
                ...
                for other_u = 1:U
                    if other_u ~= u
                        ...
                        s_other = s_u{other_u};
                        interference = g_u_other * s_other; 
...
markers = {'-o', '-s', '-d', '-^', '-v'};
for m = 1:length(modulation_schemes)
    plot(rho_values, BER_results(m, :), markers{m}, 'LineWidth', 2, 'DisplayName', modulation_schemes{m});
end
set(gca, 'YScale', 'log');
xlabel('Rho');
ylabel('Average BER');
