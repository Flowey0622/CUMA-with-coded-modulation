...
% Port settings
N1 = 10; % Fixed N1 value
N2 = 10; % Fixed N2 value
N = N1 * N2; % Total number of ports

modulation_schemes = {'BPSK', 'QPSK', '16-QAM', '64-QAM', '256-QAM'}; % Different modulation schemes
BER_results = zeros(length(modulation_schemes), length(U_values)); % Store BER results
...
for m = 1:length(modulation_schemes)
    ...
    for u_idx = 1:length(U_values)
        ...
        % Calculate average BER for this user count and modulation scheme
        average_BER_users = mean(BER_users, 2);
        average_BER = mean(average_BER_users);
        BER_results(m, u_idx) = average_BER;
    end
end
...
markers = {'-o', '-s', '-d', '-^', '-v'};
for m = 1:length(modulation_schemes)
    plot(U_values, BER_results(m, :), markers{m}, 'LineWidth', 2, 'DisplayName', modulation_schemes{m});
end
set(gca, 'YScale', 'log');
