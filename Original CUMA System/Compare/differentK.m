...
% K
K_values = [0, 10, 20, 30, 40]; % different rician factors
modulation_schemes = {'BPSK', 'QPSK', '16-QAM', '64-QAM', '256-QAM'}; % different modulations
BER_results = zeros(length(modulation_schemes), length(K_values)); 
...
for m = 1:length(modulation_schemes)
    ...
    for k_index = 1:length(K_values)
        ...
        BER_results(m, k_index) = average_BER;
    end
end
...
markers = {'-o', '-s', '-d', '-^', '-v'};
for m = 1:length(modulation_schemes)
    plot(K_values, BER_results(m, :), markers{m}, 'LineWidth', 2, 'DisplayName', modulation_schemes{m});
end
set(gca, 'YScale', 'log');
