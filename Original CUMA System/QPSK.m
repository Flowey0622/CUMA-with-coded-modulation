...
N1=10;
N2=10;%the number of ports at each user
...
numBits = 200;
numSymbols = numBits/2; % Number of symbols per user
...
for experiment = 1:numExperiments
    ...
    for n=1:Nt   
        ...
        for k = 1:numSymbols
            bits = original_bits{n}(2*k-1:2*k);
            if isequal(bits, [0 0])
                s_u{n}(k) = 1 + 1i;
        ...
        %fprintf('input signal for user %d under QPSK modulation', n);
        %disp(s_u{n});
    end
    ...
    for u = 1:U
        ...
        for m = 1:numSymbols
            ...
            % Decision based on the quadrant of the complex plane
            if real_part > 0 && imag_part > 0
                demodulated_bits{u}(2*m-1:2*m) = [0 0]; % 对应1+1i
            elseif real_part > 0 && imag_part < 0
                demodulated_bits{u}(2*m-1:2*m) = [0 1]; % 对应1-1i
            elseif real_part < 0 && imag_part > 0
                demodulated_bits{u}(2*m-1:2*m) = [1 0]; % 对应-1+1i
            else
                demodulated_bits{u}(2*m-1:2*m) = [1 1]; % 对应-1-1i
            end
        end
