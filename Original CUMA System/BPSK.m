...
numSymbols = 104; % Number of symbols per user
numBits = numSymbols; % 1 symbo 1 bit
...
for experiment = 1:numExperiments
    ...
    for n=1:Nt   
        original_bits{n} = randi([0 1], 1, numBits); % generate original bitstream
    ...
    for u = 1:U
        ...
        for m =1: numSymbols
            ...
            if real_part > 0
                demodulated_bits{u}(m) = 0;
            else
                demodulated_bits{u}(m) = 1;
            end
        end
