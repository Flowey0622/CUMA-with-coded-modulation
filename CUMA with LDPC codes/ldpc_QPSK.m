...
numBits = messageLength; % 每个符号2个比特
...
for experiment = 1:numExperiments
    ...
    for n=1:Nt   
        ...
        s_u_ldpc{n}= pskmod(encoded_bits{n},M,InputType='bit');
        s_u{n} = pskmod(original_bits{n},M,InputType='bit');

        % ldpc
        numEncodedBits = length(encoded_bits{n});
        numEncodedSymbols = numEncodedBits / 2;
    ...
    for u = 1:U
        ...
        demodulatedLLR_ldpc = pskdemod(estimated_symbols_ldpc{u}.', M, 'OutputType', 'approxllr','NoiseVariance',N0); 
...
average_BER_users_ldpc = mean(BER_users_ldpc, 2);
average_BER_users = mean(BER_users, 2);

% Display average BER for each user
for u = 1:U
    fprintf('Average BER for user %d over %d experiments with ldpc: %f\n', u, numExperiments, average_BER_users_ldpc(u));
    fprintf('Average BER for user %d over %d experiments: %f\n', u, numExperiments, average_BER_users(u));

    improvement_percentage = ((average_BER_users(u) - average_BER_users_ldpc(u)) / average_BER_users(u)) * 100;
    fprintf('Improvement in BER for user %d due to LDPC: %.2f%%\n', u, improvement_percentage);
end
