...
BER_results_64QAM = zeros(2, length(U_values)); % Store BER results
K=0;
L=100;
...
for u_idx = 1:length(U_values)
    U = U_values(u_idx); % Number of user       
    Nt = U; % Number of transmit antennas (base station) should be equal to the current U value
    ...
    average_BER_users_turbo = mean(mean(BER_users_turbo, 2));
    average_BER_users = mean(mean(BER_users, 2));

    BER_results_64QAM(1, u_idx) = average_BER_users_turbo;
    BER_results_64QAM(2, u_idx) = average_BER_users; 
end
