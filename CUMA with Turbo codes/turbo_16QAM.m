...
for experiment = 1:numExperiments
    ...
    for n=1:Nt   
        ...
        s_u_turbo{n}=lteSymbolModulate(coded_bits{n},'16QAM');
        s_u{n} = qammod(original_bits{n},M,InputType='bit');

 
        numSymbols =length(original_bits{n})/4;

        % turbo
        numCodedSymbols = length(coded_bits{n})/4; 
    ...
    for u = 1:U
        ...
        softBits = lteSymbolDemodulate(estimated_symbols_turbo{u},'16QAM','Soft');
