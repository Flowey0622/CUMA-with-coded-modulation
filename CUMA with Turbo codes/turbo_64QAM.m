...
for experiment = 1:numExperiments
    ...
    for n=1:Nt   
        ...
        coded_bits{n} = lteTurboEncode(original_bits{n}');
    
        s_u_turbo{n}=lteSymbolModulate(coded_bits{n},'64QAM');
        s_u{n} = qammod([original_bits{n}; zeros(4, 1)],M,InputType='bit');

        numSymbols =(length(original_bits{n})+4)/6;

        % turbo
        numCodedSymbols = length(coded_bits{n})/6; 
    ...
    for u = 1:U
        real_noise = sqrt(noise_power / 2) * randn(N, numCodedSymbols); 
        imag_noise = sqrt(noise_power / 2) * randn(N, numCodedSymbols); 
        eta_turbo{u} = real_noise + 1i * imag_noise; 
        eta{u} = eta_turbo{u}(:, 1:numSymbols);
    end
    total_interference = cell(1,U);
    total_interference_ldpc = cell(1,U);
    total_interference_turbo = cell(1,U);
    ...
    for u = 1:U
        ...
        s_u_I_turbo = real(s_u_turbo{u}); % 1*100
        s_u_Q_turbo = imag(s_u_turbo{u});
        ...
        r_I1_turbo = A * s_u_I_turbo - a * s_u_Q_turbo; % 100*1
        r_Q1_turbo = a * s_u_I_turbo + A * s_u_Q_turbo;
        r_I2_turbo = b * s_u_I_turbo - B * s_u_Q_turbo;
        r_Q2_turbo = B * s_u_I_turbo + b * s_u_Q_turbo;
        ...
        total_interference_turbo{u}= eta_turbo{u};
        total_interference{u}= eta{u};

        for other_u = 1:U
            if other_u ~= u
                g_u_other = H{u} * precoding_vectors{other_u}; % 100*1
                s_other_turbo = s_u_turbo{other_u};
                interference = g_u_other * s_other_turbo.'; % Cumulative interference
                total_interference_turbo{u} = total_interference_turbo{u} + interference;
            end
        end
        ...
        noise_I1_turbo =sum(real(total_interference_turbo{u}(K1{u},:)));
        noise_Q1_turbo =sum(imag(total_interference_turbo{u}(K1{u},:)));
        noise_I2_turbo =sum(real(total_interference_turbo{u}(K2{u},:)));
        noise_Q2_turbo =sum(imag(total_interference_turbo{u}(K2{u},:)));
        ...
        received_signal_I1_turbo{u} = r_I1_turbo + noise_I1_turbo.';
        received_signal_Q1_turbo{u} = r_Q1_turbo + noise_Q1_turbo.';
        received_signal_I2_turbo{u} = r_I2_turbo + noise_I2_turbo.';
        received_signal_Q2_turbo{u} = r_Q2_turbo + noise_Q2_turbo.';
        ...
        r_u_turbo{u} = [received_signal_I1_turbo{u}.'; 
                       received_signal_Q1_turbo{u}.';   
                       received_signal_I2_turbo{u}.'; 
                       received_signal_Q2_turbo{u}.'];  % 4*numsymbols
        ...
        estimated_symbols_turbo{u} = G_u_u \ r_u_turbo{u};

        estimated_symbols{u}= estimated_symbols{u}(1, :)+ 1i *estimated_symbols{u}(2,:);
        estimated_symbols_turbo{u}= estimated_symbols_turbo{u}(1, :)+ 1i *estimated_symbols_turbo{u}(2,:);
 
        demodulated_bits{u} = qamdemod(estimated_symbols{u}.',M,OutputType='bit');
        
       
        softBits = lteSymbolDemodulate(estimated_symbols_turbo{u},'64QAM','Soft');
        decoded_bits_int8 = lteTurboDecode(softBits,30);
        decoded_bits_turbo{u} = double(decoded_bits_int8);
        

        % Calculate number of bit errors
        num_errors_turbo = sum(original_bits{u} ~= decoded_bits_turbo{u});
        num_errors = sum([original_bits{n}; zeros(4, 1)] ~= demodulated_bits{u});
        
        % Calculate BER
        BER_turbo(u) = num_errors_turbo / numBits;
        BER(u) = num_errors / (numBits+4);
        
         % Calculate BER for each user    
         BER_users_turbo(u, experiment) = BER_turbo(u);
...
average_BER_users_turbo = mean(BER_users_turbo, 2);
average_BER_users = mean(BER_users, 2);

% Display average BER for each user
for u = 1:U  
    fprintf('Average BER for user %d over %d experiments with Turbo: %f\n', u, numExperiments, average_BER_users_turbo(u));
    fprintf('Average BER for user %d over %d experiments: %f\n', u, numExperiments, average_BER_users(u));
   
    improvement_percentage = ((average_BER_users(u) - average_BER_users_turbo(u)) / average_BER_users(u)) * 100;
    fprintf('Improvement in BER for user %d due to Turbo: %.2f%%\n', u, improvement_percentage);
end
