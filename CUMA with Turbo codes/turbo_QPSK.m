...
for experiment = 1:numExperiments
    ...
    for n=1:Nt   
        ...
        s_u_turbo{n}=lteSymbolModulate(coded_bits{n},'QPSK');
        s_u{n} = pskmod(original_bits{n},M,InputType='bit');

 
        numSymbols =length(original_bits{n})/2;

        % turbo
        numCodedSymbols = length(coded_bits{n}) / 2; 
    ...
    for u = 1:U
        ...
        demodulated_bits{u} = pskdemod(estimated_symbols{u}.',M,OutputType='bit');
        
       
        softBits = lteSymbolDemodulate(estimated_symbols_turbo{u},'QPSK','Soft');
